...
from network.blocks.basic_unit import _ConvINReLU3D
from network.blocks.process_block import InputLayer, OutputLayer
from network.blocks.residual_block import ResTwoLayerConvBlock, ResFourLayerConvBlock
from network.blocks.unet_block import UnetTwoLayerBlock
...
class UNet(nn.Module):
    ...
    def __init__(self, cfg=None):
        ...
        if cfg['ENCODER_CONV_BLOCK'] == 'UnetTwoLayerBlock':
            encoder_conv_block = UnetTwoLayerBlock
        elif cfg['ENCODER_CONV_BLOCK'] == 'ResTwoLayerConvBlock':
            encoder_conv_block = ResTwoLayerConvBlock
        else:
            encoder_conv_block = ResFourLayerConvBlock
        
        if cfg['DECODER_CONV_BLOCK'] == 'UnetTwoLayerBlock':
            decoder_conv_block = UnetTwoLayerBlock
        elif cfg['DECODER_CONV_BLOCK'] == 'ResTwoLayerConvBlock':
    ...
    def _initialize_weights(self):
        for m in self.modules():  # 遍历网络的每个模块
            if isinstance(m, nn.Conv3d):  # 判断是否是3D卷积层
                nn.init.kaiming_normal_(m.weight, nonlinearity='relu')  # 权重参数初始化
                if m.bias is not None:  # 卷积层存在偏置项时设置为0
                    m.bias.data.zero_()
    ...
    def forward(self, x):
        ...
        if self.is_preprocess:
            x = self.input(x)   # 进行预处理

        x = self.conv0_0(x)                  # 第一层，两次卷积
        x1_0 = self.conv1_0(self.pool(x))   # 第二层，先池化再卷积
